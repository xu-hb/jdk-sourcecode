 ...
 * 2. 运行null
 *
 * 将设元素合理分配在桶中，(<tt>get</tt> and <tt>put</tt>)的时间复杂度是O(1)
 *
 * 有两个参数影响HashMap性能：
 ...
 * 同步容器：<pre> Map m = Collections.synchronizedMap(new HashMap(...));</pre>
 *
...
public class HashMap<K,V> extends AbstractMap<K,V>
    ...
    /**
     * 链表转换为树的最小容量
     * 应该至少是 4 * TREEIFY_THRESHOLD，以避免resize和树化阈值之间的冲突
     */
    ...
    /**
     * 扰动函数
     * 哈性表的容量都是2的N次方，元素的hashCode()很多时候下低位是相同的
     * 因此使用高半位和低半位运算，使低半位也有高半位的信息
     */
     ...
     * 如果不存在则返回null
     * 也可能key映射的值本身是null
     * {@link #containsKey containsKey}可以区分两种情况
     *
        ...
        if ((p = tab[i = (n - 1) & hash]) == null)
            ...
            if (p.hash == hash &&
                ...
                for (int binCount = 0; ; ++binCount) {
                    //不断遍历，直到：1.匹配到对应key
                    //2.未匹配，遍历到链表尾,用尾插法在尾部插入新节点，同时判断是否需要转换红黑树
                    if ((e = p.next) == null) {
